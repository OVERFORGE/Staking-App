    ...
    if (address) {
      ...
      for (let i = 0; i < length; i++) {
        try {
          ...
          console.log(`Pool ${i} raw poolInfo:`, poolInfo);
          ...
          const pool = {
            ...
            depositAmount: toEth(poolInfo.depositedAmount.toString() || "0"),
          ...
          console.log(`Failed to load pool data for index ${i}:`, e);
        }
      }
      ...
      const data = {
        ...
        depositToken,
        poolInfoArray,
        totalDepositArray: totalDepositAmount,
...
export const UPDATE_TOKEN = async (_address) => {
  try {
    ...
    notifySuccess("Calling the contract");
    const contract = await TOKEN_ICO_CONTRACT();
